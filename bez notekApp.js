...
// Enhanced Training Calendar Component with Note Feature
          ...
          return (
            <TouchableOpacity
              ...
              style={[
                ...
                hasNote && { backgroundColor: "#f87171", borderColor: "#f87171" },
              ]}
            ...
            >
              ...
              {visibleNotes?.[dateStr] && hasNote && (
                <View style={{
                  ...
                  <Text style={{ color: "#b91c1c", fontSize: 13 }}>{calendarNotes[dateStr]?.text}</Text>
                </View>
              )}
            </TouchableOpacity>
          );
  ...
  const generateTips = () => {
    ...
    if (dogBreed && breedTips[dogBreed]) {
      tips.push({
        ...
        title: `${dogBreed} Tip`,
        content:
          breedTips[dogBreed][Math.floor(Math.random() * breedTips[dogBreed].length)],
      });
    }
...
// Enhanced Photo Progress Component with Base64 conversion
const PhotoProgress = ({ week, photos, onPhotoAdded }) => {
  const takePhoto = async () => {
    ...
    if (!result.canceled) {
      try {
        // Convert image to base64 for better cross-device compatibility
    ...
    <View style={styles.photoContainer}>
      ...
      <View style={styles.photosGrid}>
        {weekPhotos.map((photo, index) => {
          // Determine the source based on whether we have base64 data or uri
          const source = photo.base64 
            ? { uri: `data:image/${photo.type || 'jpeg'};base64,${photo.base64}` }
            : { uri: photo.uri };
            
          return <Image key={index} source={source} style={styles.photo} />;
        })}
      </View>
    </View>
  );
...
const AchievementBadge = ({ achievement, unlocked }) => (
  <View style={[styles.achievementBadge, unlocked && styles.achievementUnlocked]}>
    <Feather name={achievement.icon} size={28} color={unlocked ? "#f59e0b" : "#9ca3af"} />
...
function MainApp() {
  ...
  // New state for manual sync button
  ...
  const forceSyncToFirebase = useCallback(async () => {
    if (!familyId) {
      showToast("No family ID set", "error");
      return;
    }
    ...
    try {
      ...
      if (docSnap.exists) {
        ...
        if (data.dogName && data.dogBreed && data.dogDob) {
          ...
          Alert.alert(
            "Family ID Exists",
            "This Family ID exists but is missing some dog info. Please contact your admin."
          );
          setCheckingFamilyId(false);
        }
      } else {
  ...
  const toggleDailyActivity = useCallback((timeSlot, activity) => {
    ...
    setCompletedDailyByDate((prev) => {
      ...
      return {
        ...prev,
        [today]: isCompleted ? prevForToday.filter((k) => k !== key) : [...prevForToday, key],
      };
    });
  }, []);
  ...
  const handleSaveFamilyMember = useCallback((id, newName) => {
    setFamily((family) =>
      family.map((f) => (f.id === id ? { ...f, name: newName || "Unnamed", editing: false } : f))
    );
  ...
  const openEditDogModal = useCallback(() => {
    try {
      // Fix for issue 1: Dog edit button crashes app
      // Properly initialize edit modal states with current values or defaults
  ...
  const navigationTabs = [
    { key: "daily", iconLib: Feather, iconName: "home" },
    { key: "weekly", iconLib: Feather, iconName: "calendar" },
    { key: "progress", iconLib: Feather, iconName: "bar-chart" },
    ...
    if (!showDogInput) {
      ...
      return (
        <View style={styles.setupContainer}>
          ...
          <Text style={styles.setupText}>Set your Family ID and Dog's Details (only needed once).</Text>
          ...
          <View style={styles.headerContainer}>
            <View style={styles.headerTop}>
              ...
              <View style={styles.dateTimeContainer}>
                ...
                {/* Adding the new Force Sync button */}
                <TouchableOpacity 
            ...
            {navigationTabs.map((tab) => {
              ...
              return (
                <TouchableOpacity
                  ...
                  activeOpacity={0.7}
                >
                  <IconComponent name={tab.iconName} size={22} color={isActive ? "#ffffff" : "#64748b"} />
                  {isActive && (
                    <View
                      style={{
                        position: "absolute",
            ...
            <View>
                    ...
                    {acts.map((activity, idx) => (
                      ...
                      >
                        {completedToday.includes(`daily-${timeSlot}-${activity}`) ? (
                          <MaterialCommunityIcons name="checkbox-marked-circle" color={colors.iconColor} size={22} />
                        ) : (
              ...
              <View style={styles.section}>
                ...
                {(weeklyPlans[currentWeek] || []).map((activity, idx) => (
                  <TouchableOpacity key={idx} style={styles.activityRow} onPress={() => toggleActivity(currentWeek, activity)} activeOpacity={0.8}>
          ...
          {viewMode === "weekly" && (
            <View style={styles.section}>
              ...
              >
                ...
                >
                  🎯 {trainingStages[currentStage]?.name} &nbsp;&bull;&nbsp; {trainingStages[currentStage]?.range}
                </Text>
              </View>
                  ...
                  return (
                    <View key={week} style={styles.weekHistoryItem}>
                      ...
                      <View style={styles.progressBarContainer}>
                        <View style={[styles.progressBar, { width: `${weekRate * 100}%` }]} />
                      </View>
        ...
        <View style={styles.quickNoteOverlay}>
          <View style={styles.quickNoteModal}>
            ...
            <View style={styles.quickNoteActions}>
              <TouchableOpacity style={styles.quickNoteCancel} onPress={() => setShowQuickNote(false)}>
                <Text style={styles.quickNoteCancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.quickNoteSave} onPress={handleQuickNoteSubmit}>
        ...
        <View style={{
          ...
          <View style={{
            ...
            <TextInput
              ...
              placeholder="Note..."
              style={{ borderWidth: 1, borderColor: "#ddd", borderRadius: 8, padding: 10, minHeight: 60 }}
              multiline
            />
            <View style={{ flexDirection: "row", marginTop: 16, justifyContent: "flex-end" }}>
              ...
              >
                <Text style={{ color: "#3b82f6", fontWeight: "bold" }}>Save Note</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  // New style for force sync button
  forceSyncButton: {
    flexDirection: "row",
    alignItems: "center",
    backgroundColor: "#f0f9ff",
